...
int main(int argc, char **argv) {
  ...
  if (!parse_search_cli(argc, argv, "three_d2", options)) {
  ...
  if constexpr (N <= 16) {
    ...
    if (options.use_frontier) {
      return solve_with_device_stack_d2<N>(options.frontier_min_on);
    }
    if (options.has_seed) {
      return solve_with_device_stack_d2<N>(&seed_on, &seed_off);
    }
  } else {
  ...
  return solve_with_device_stack_d2<N>();
}
