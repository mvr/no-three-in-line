...
__global__ void init_d2_line_table_kernel_32(uint32_t *table) {
  ...
  constexpr unsigned cell_count = N * full_n;
  constexpr unsigned line_rows = ((N + 7u) & ~7u);
  ...
  const unsigned px = p_idx % full_n;
  const unsigned py = p_idx / full_n;
  const unsigned qx = q_idx % full_n;
  const unsigned qy = q_idx / full_n;
...
__global__ void init_d2_line_table_kernel_64(uint64_t *table_even, uint64_t *table_odd) {
  constexpr unsigned full_n = 2 * N;
  constexpr unsigned cell_count = N * full_n;
  constexpr unsigned line_rows = ((((N + 1u) >> 1) + 7u) & ~7u);
  const unsigned pair_idx = blockIdx.x;
  if (pair_idx >= cell_count * cell_count) {
    return;
  }
  ...
  if (lane < line_rows) {
    ...
    table_even[idx] = (static_cast<uint64_t>(mask.state.y) << 32) | static_cast<uint64_t>(mask.state.x);
    table_odd[idx] = (static_cast<uint64_t>(mask.state.w) << 32) | static_cast<uint64_t>(mask.state.z);
  }
}
...
struct D2Traits {
  ...
  static void init_line_table_host() {
    ...
    if constexpr (W == 32) {
      ...
      init_d2_line_table_kernel_64<N><<<static_cast<unsigned>(total_entries), 32>>>(d_table_even, d_table_odd);
      cudaDeviceSynchronize();
      cudaMemcpyToSymbol(g_d2_line_table_64_even, &d_table_even, sizeof(d_table_even));
      cudaMemcpyToSymbol(g_d2_line_table_64_odd, &d_table_odd, sizeof(d_table_odd));
    }
  }
