...
template <unsigned N, unsigned W>
int solve_with_device_stack_d2(const SearchOptions<W> &options) {
  if constexpr (W == 32 && N > 16) {
    (void)options;
    std::cerr << "[d2] W=32 is invalid for N > 16\n";
    return 1;
  } else {
    if (options.mode == SearchMode::Frontier) {
      return solve_with_device_stack_impl<D2Traits<N, W>, true>(options);
    }
    return solve_with_device_stack_impl<D2Traits<N, W>, false>(options);
  }
}

template int solve_with_device_stack_d2<N, 32>(const SearchOptions<32> &);
template int solve_with_device_stack_d2<N, 64>(const SearchOptions<64> &);
